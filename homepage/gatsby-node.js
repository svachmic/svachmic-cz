...
async function fetchGitLabContributions(reporter) {
  try {
    const res = await fetch(
      'https://gitlab.com/users/svachmic/calendar.json'
    )
    if (!res.ok) {
...
async function fetchFeaturedRepoStats(reporter) {
  ...
  try {
    ...
    if (!res.ok) {
      reporter.warn(`GitHub GraphQL API returned ${res.status} for featured repo`)
      return FEATURED_REPO_DEFAULTS
    }
    ...
    const loc = Math.round(totalBytes / 35 / 1000) * 1000 || FEATURED_REPO_DEFAULTS.loc
    const commits =
      repo.defaultBranchRef?.target?.history?.totalCount ||
      FEATURED_REPO_DEFAULTS.commits
    const license =
      repo.licenseInfo?.spdxId || FEATURED_REPO_DEFAULTS.license
...
exports.sourceNodes = async ({
  ...
  if (process.env.STUB_NOW === 'true') {
    nowText = 'Přemlouvám Gatsby a Netlify ke spolupráci. Na všechno ostatní je tady...Claude 🧠😬'
  ...
  createNode({
    ...
    internal: {
      type: 'NowStatus',
      contentDigest: createContentDigest({ text: nowText, date: nowDate }),
    },
  })
